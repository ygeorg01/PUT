...
# Helper Functions
###############################################################################
# equirectangular_convolutions = True
equirectangular_convolutions = False
...
class PatchSampleF(nn.Module):
    ...
    # def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=15, gpu_ids=[]):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        # print('feats: ', feats[0].shape, feats[1].shape, feats[2].shape, feats[3].shape, feats[4].shape)
        for feat_id, feat in enumerate(feats):
            B, H, W = feat.shape[0], feat.shape[2], feat.shape[3]
            # print('feat shape: ',feat.shape)
            # print('Input feat before flatten: ', feat.shape)
            feat_reshape = feat.permute(0, 2, 3, 1).flatten(1, 2)
            # print('Input feat before reshape: ', feat_reshape.shape)
            ...
            if num_patches > 0:
                if patch_ids is not None:
                    ...
                    #print('Patch id1: ', patch_id.shape)
                    patch_id = patch_id[:int(min(num_patches, patch_id.shape[0]))]  # .to(patch_ids.device)
                    #print('Patch id2: ', patch_id.shape)
                #print('Feat before reshape/ patch_id: ', feat_reshape.shape, patch_id.shape)
            ...
            if self.use_mlp:
                ...
                # print('MLP input: ', x_sample.shape)#, x_sample)
                x_sample = mlp(x_sample)
                # print('MLP output: ', x_sample.shape)
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        if equirectangular_convolutions:
            print('....Create first conv layer....')
            model = [nn.ReflectionPad2d(3),
                 EquiConv2d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias),
                 norm_layer(ngf),
                 nn.ReLU(True)]
            print('....First conv layer created....')
        else:
            model = [nn.ReflectionPad2d(3),
        ...
        if len(layers) > 0:
            ...
            for layer_id, layer in enumerate(self.model):
                ...
                if layer_id in layers:
                    ...
                    #print('In G, id/feat shape: ', layer_id, feat.shape)
                    feats.append(feat)
                else:
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        if(no_antialias):
            if equirectangular_convolutions:
                sequence = [EquiConv2d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw), nn.LeakyReLU(0.2, True)]
            else:
                sequence = [nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw), nn.LeakyReLU(0.2, True)]
        else:
            if equirectangular_convolutions:
                sequence = [EquiConv2d(input_nc, ndf, kernel_size=kw, stride=1, padding=padw), nn.LeakyReLU(0.2, True), Downsample(ndf)]
            else:
